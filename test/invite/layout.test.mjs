...
for (const width of WIDTHS) for (const slug of SLUGS) {
  test(`${slug} at ${width}px: no overflow, no broken requests, content on screen`, async () => {
    ...
    const m = await page.evaluate(() => {
      ...
      // a cover section (nothing but bleeds) is fitted to the screen's height and crops its sides by design
      const content = secs.filter((s) => !('cover' in s.dataset)).map((s) => { const k = parseFloat(getComputedStyle(s.querySelector('.stage')).transform.match(/matrix\(([^,]+),/)[1]); const st = s.querySelector('.stage').getBoundingClientRect(); return { left: st.left + (+s.dataset.cl) * k, right: st.left + (+s.dataset.cl + +s.dataset.cw) * k }; });
