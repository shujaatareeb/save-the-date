...
const scrollBy = (page, y) => page.evaluate((y) => {
...
const measureScrollHeight = (page) => page.evaluate(() => {
  const all = [document.scrollingElement, ...document.querySelectorAll('body *')];
  const sc = all.find((e) => e && e.scrollHeight > e.clientHeight + 50 && e.clientHeight >= innerHeight * 0.6) || document.scrollingElement;
...
const unclip = (page) => page.evaluate(() => {
  const all = [document.scrollingElement, ...document.querySelectorAll('body *')];
  const sc = all.find((e) => e && e.scrollHeight > e.clientHeight + 50 && e.clientHeight >= innerHeight * 0.6) || document.scrollingElement;
  let el = sc;
  while (el) {
...
for (const [slug, n] of Object.entries(PAGES)) {
  ...
  let h = await scrollBy(page, 0);
  for (let y = 0; y < h; y += 600) { h = await scrollBy(page, y); await page.waitForTimeout(700); }
  await page.waitForTimeout(3000);
  await scrollBy(page, 0);
  await page.evaluate(() => document.querySelectorAll('footer, [class*="footer"], main ~ *').forEach((f) => f.remove()));
  const measuredHeight = await measureScrollHeight(page);
  await unclip(page);
  await page.waitForTimeout(300);
  await page.screenshot({ path: path.join(OUT, `${slug}.png`), ...referenceScreenshotOptions(measuredHeight) });
  console.log(slug, 'captured', `${measuredHeight}px`);
  await page.close();
}
await browser.close();
